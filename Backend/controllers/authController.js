...
const signupUser = async (req, res) => {
  ...
  const user = await userModel.signup(u_name, email, password);
  console.log("signupuserID", user._id);
  const linkToken = createToken(user._id, "2m");
  const verificationLink = `http://localhost:4000/auth/verify-email?linkToken=${linkToken}`;
  ...
  return res
    .status(200)
    .json({ linkToken, message: "link sent to email", success: true });
};
